...
public class Flink05_Window_ProcessWindowFunc {
    public static void main(String[] args) throws Exception {
        //TODO 使用process测试全量聚合
        //1.获取执行环境
        ...
        SingleOutputStreamOperator<Tuple3<Long, String, Long>> processDS = windowedStream.process(new ProcessWindowFunction<Tuple2<String, Long>, Tuple3<Long, String, Long>, String, TimeWindow>() {

            @Override
            public void process(String key, Context context, Iterable<Tuple2<String, Long>> elements, Collector<Tuple3<Long, String, Long>> out) throws Exception {
                ArrayList<Tuple2<String, Long>> list = Lists.newArrayList(elements.iterator());

                Long size = Long.valueOf(list.size());

                out.collect(Tuple3.of(context.window().getStart(),
                        key, size));
            }
        });
